...
module.exports = class Generate {
  ...
  async process() {
    ...
    if (entityType === "package") await Generate.generatePackage(name);
      ...
      await Generate.generateComponent(name, pkgName, config, entityType, this.config.getWorkspaceLibraryList());
    } else {
    ...
    if (templateType === "uu5-lib") {
      ...
      if (!hasExampleConfig) {
        ...
        for (let k in config.externalsApp) {
          if (k === "systemjs" || k === "uu_oidcg01") continue;
...
async function prettify(content, destFile) {
  try {
    ...
    return prettier.format(content, { filepath: destFile });
  } catch (e) {
