...
class Finder {
  ...
  static findInModules(root = process.cwd(), globPattern = "") {
    ...
    if (isPnpm(root)) {
      let partialRes = glob.sync(`${root}/node_modules/*/*/*/node_modules/*/${globPattern}`, globOpts);
      if (!partialRes || partialRes.length === 0) {
        // pnpm 5.x changed structure of installed modules
        partialRes = glob.sync(`${root}/node_modules/*/*/node_modules/*/${globPattern}`, globOpts);
      }
      result = result.concat(partialRes);
    } else {
      result = result.concat(glob.sync(`${root}/node_modules/*/${globPattern}`, globOpts));
    }
  ...
  static findPackage(root = process.cwd(), name, throwOnMultipleResults = true) {
    ...
    if (matchingExternalModules && matchingExternalModules.length) {
      ...
      if (!results || results.length === 0) {
        // pnpm 5.x changed structure of installed modules
        results = glob.sync(`${root}/node_modules/*/*/node_modules/${name}/`, globOpts);
      }
