...
function getWebpackConfig(options, env) {
  ...
  //   }
  // };
  let babelConfig = require("./.babelrc.js")(mode, null, opts.moduleType === "cjs");
  ...
  let rules = [
    {
      oneOf: [
        ...
        { exclude: /\.(jsx?|mjs|json)$/, use: [fileLoader] }, // if import-ing anything else just copy it
      ],
    },
  ];
  ...
  if (opts.copyFiles) {
    ...
    htmlPostProcessedFiles.forEach(function (htmlFile) {
      ...
      if (!processorV1) {
        ...
        processorV1 = new HtmlPostProcessorV1({
          ...
          usesSystemJs: !!(pkg.dependencies || {}).systemjs,
        });
      ...
      plugins.push(
        new HtmlWebpackPlugin({
          ...
          ejsFixUrls: (htmlSnippet) =>
            ...
            processorV1.processHead(htmlSnippet, { file: htmlFile, assets: data1.compilation.assets, depCache: !env.isDevServer }),
        })
      );
      let data2 = {}; // :-(
      plugins.push(
        new HtmlWebpackPlugin({
          ...
          ejsFixUrls: (htmlSnippet) =>
            // TODO Generate depCache also when developing, i.e. add watching of index.js (htmlFile.js) for changes.
            processorV2.processHead(htmlSnippet, { file: htmlFile, assets: data2.compilation.assets, depCache: !env.isDevServer }),
        })
      );
    });
    ...
    if (opts.includeDemoPages) {
      cwpSettings.push({ from: "**/demo/**" });
      cwpSettings.push({ context: path.resolve("."), from: "demo/**" });
    }
  ...
  let sameVersionLibs = env.sameProductFrontendLibs.concat(Object.keys(submodules));
  if (opts.moduleType === "umd" && targetEnvironment === "web" && sameVersionLibs.length > 0) {
    sameVersionLibs = [...new Set(sameVersionLibs)];
    ...
    versionCheckContent = versionCheckContent.replace(/<%=\s*WORKSPACE_LIBS\s*%>/g, () =>
      JSON.stringify(sameVersionLibs)
    );
    ...
    for (let name of sameVersionLibs) {
      ...
      let content = `let ex = require(${JSON.stringify(name + "--version-check")});
let versionCheck = require(${JSON.stringify(versionCheckRelPath)});
if (ex?.name === ${JSON.stringify(name)}) versionCheck(ex?.name, ex?.version);
module.exports = ex;
`;
    ...
    let watchedImportsRegExp = new RegExp(`^(${sameVersionLibs.map((it) => StringUtil.regexpQuote(it)).join("|")})$`);
...
function getStyleLoaderInsertInto(dataOwner, emotionKey, srcAbsPath, configName) {
  ...
  let modulePath = "__project__/" + path.relative(srcAbsPath, filePath).replace(/\\/g, "/").replace(/^\.\//, "");
