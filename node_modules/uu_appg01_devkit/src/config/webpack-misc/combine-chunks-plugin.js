  ...
  apply(compiler) {
    ...
    compiler.hooks.compilation.tap("CombineChunksPlugin", (compilation) => {
      compilation.hooks.optimizeChunksAdvanced.tap("CombineChunksPlugin", (chunks) => {
        ...
        if (verbose) {
          console.log(
            `------- CombineChunksPlugin - combining small chunks into bigger one limiting to ${Math.round(
              combinedMaxSizeUnminified / 1000
            )} kB unminified, ${Math.round(combinedMaxSize / 1000)} kB minified (approximation).`
          );
          console.log(
            compilation.name + " - initial chunks:\n" + chunks.map((chunk) => "  " + prettyPrintChunk(chunk)).join("\n")
          );
        }
        ...
        let items = chunks
          ...
          .sort(
            (a, b) =>
              (a.chunk.entryModule ? 1 : 0) - (b.chunk.entryModule ? 1 : 0) ||
          ...
          if (changedMap.size > 0) {
            console.log(
              "\n" +
                ...
                " - final chunks:\n" +
                chunks
                  .map((chunk) => "  " + prettyPrintChunk(chunk, changedMap.has(chunk) ? " changed" : " not changed"))
                  .join("\n") +
                "\n"
            );
          } else {
...
function prettyPrintChunk(chunk, extra = "") {
  ...
  return `${chunk.debugId}, ~${(Math.round(size / 1000) + "").padStart(
    3,
    " "
  )} kB, ${chunk.getNumberOfGroups()} refs,${extra} ${entry}[${modules.join(", ")}]`;
}
