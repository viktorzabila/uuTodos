...
class HtmlPostProcessorV1 {
  constructor(config) {
    this._config = config;
    if (this._config.usesSystemJs && !warnedUsesSystemJs) {
      warnedUsesSystemJs = true;
      console.warn(
        require("chalk").yellow("WARNING") +
          " Your *.html UVEs are using deprecated SystemJS loader. Use Uu5Loader instead - see upgrade instructions for uuAppDevKit 4.3.0 in https://uuapp.plus4u.net/uu-bookkit-maing01/e884539c8511447a977c7ff070e7f2cf/book/page?code=70235938"
      );
    }
  }
  ...
  processHead(template, opts = {}) {
    ...
    let loaderConfigSnippet = this._getLoaderHtmlSnippet(loaderSettings);
  ...
  getLoaderSettings(externals, opts) {
    ...
    deps.forEach((depName) => {
      ...
      settings.imports[depName] = depPath;
      ...
      if ("dependencyMap" in config) {
        let { dependencyMap: itemDependencyMap } = config;
        delete config.dependencyMap;
    ...
    if (
      this._config.usesSystemJs &&
      (hasLibs || hasUu5Libs) &&
      !Object.values(settings.meta).some((value) => value && value.deps)
    ) {
      if (hasLibs) {
    ...
    if (opts && opts.depCache && opts.file && !settings.imports["_index"] && Object.keys(dependencyMap).length > 0) {
      ...
      if (generatedJsFileAsset) {
        ...
        if (filteredDeps.length > 0) {
          ...
          settings.dependencyMap = { _index: this._makeDepsTransitiveClosureWithOrder(filteredDeps, dependencyMap) };
          settings.imports["_index"] = relUrl[0] !== "." ? "./" + relUrl : relUrl;
        }
      }
    }
  ...
  stringifyLoaderSettings(loaderSettings) {
    let result;
    if (this._config.usesSystemJs) {
      if (Object.keys(loaderSettings.dependencyMap || {}).length > 0) {
        // make dependencyMap arrays be on 1 line, i.e.:
        // dependencyMap: {
        //   "uu5g04-forms": ["uu5g04","uu5g04-bricks","react",...],
        //   "uu5g04-hooks": ["uu5g04","react",...],
        // }
        let { dependencyMap, imports, ...restSettings } = loaderSettings;
        if (imports) restSettings.paths = imports;
        restSettings.depCache = {};
        for (let k in dependencyMap) restSettings.depCache[k] = `%%%% ${k} %%%%`;
        result = JSON.stringify(restSettings, null, "  ").replace(/"%%%% ([^%]+) %%%%"/g, (m, g) =>
          JSON.stringify(dependencyMap[g])
        );
      } else {
        let { imports, ...restSettings } = loaderSettings;
        if (imports) restSettings.paths = imports;
        result = JSON.stringify(restSettings, null, "  ");
      }
    } else {
      if (Object.keys(loaderSettings.dependencyMap || {}).length > 0) {
        // make dependencyMap arrays be on 1 line, i.e.:
        ...
        //   "uu5g04-hooks": ["uu5g04","react",...],
        // }
        let { dependencyMap, ...restSettings } = loaderSettings;
        ...
        result = JSON.stringify(loaderSettings, null, "  ");
      }
    }
    return result;
  }
...
${result}`;
  ...
  _getLoaderHtmlSnippet(loaderSettings) {
    // clean-up (remove empty keys / values)
    for (let k in loaderSettings) {
      let v = loaderSettings[k];
      if (v == null || (typeof v == "object" && Object.keys(v).length == 0)) delete loaderSettings[k];
    }
    ...
    let { meta } = loaderSettings;
    if (meta) {
      loaderSettings.meta = META_PLACEHOLDER;
    ...
    let loaderVariable = this._config.usesSystemJs ? "SystemJS" : "Uu5Loader";
    let configSnippet = `window.${loaderVariable} && ${loaderVariable}.config(${this.stringifyLoaderSettings(
      loaderSettings
    )});`;
  ...
  _filterDirectDeps(directDeps, externals) {
    return directDeps
      .filter((it) => it in externals && it !== "module" && it !== "undefined") // exclude meta dependencies (filled during runtime) / undefined
      .sort();
  }

  _makeDepsTransitiveClosureWithOrder(deps, dependencyMap) {
    function addPostOrder(dep, processedSet, result) {
      if (processedSet.has(dep)) return;
      processedSet.add(dep);
      let candidates = dependencyMap[dep];
      if (candidates) {
        for (let candidate of candidates) addPostOrder(candidate, processedSet, result);
      }
      result.push(dep);
    }
