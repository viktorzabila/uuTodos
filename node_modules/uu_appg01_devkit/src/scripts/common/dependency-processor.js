...
module.exports = class DependencyProcessor {
  ...
  getDetailedExternals(type) {
    ...
    if (type === EXTERNALS_TYPE_APP) {
      ...
      if (!externals["systemjs"]) {
        let pkg = Package.getSingletonSync("package.json").get();
        if ((pkg.dependencies || {}).systemjs) {
          externals["systemjs"] = { value: config["systemjs"] };
        }
      }
    }
  ...
  _computeExternals(packageJsonFile, externalsType, meta = undefined, parentMeta = null, cumulativeResult = {}) {
    ...
    Object.keys(explicitExternals).forEach(externalName => {
      ...
      let { value, cdnVersion, cdnVersionExact } =
        parentMeta ||
        typeof externalValue !== "object" ||
        (externalsType === EXTERNALS_TYPE_APP && meta.json.spec && meta.json.spec.match(/^(uu5-lib|iso-lib|lib)/)) // we're in top-level package.json of a library but are requesting "app"-type externals (e.g. when generating demo page)
