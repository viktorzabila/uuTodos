...
module.exports = class Test {
  ...
  async process() {
    ...
    for (let { name, pkg, type } of modules) {
      if (!pkg.scripts.testBrowser || resolveByTemplateType(path.join(__dirname, "test-browser.js"), type) == null) {
        continue;
      }
      try {
        await exec("npm run testBrowser -- " + this.config.toCommandLineArgs().join(" "), {
