  ...
  process(request, response, next) {
    if (request.url.match(/\/(?:demo|doc)\/.*\.html(\?.*)?$/)) {
      ...
      if (fs.existsSync(file)) {
        ...
        // configure UU5.Environment.cdnBaseUri (before </head> or after script linking uu5-environment.js)
        modHtml = modHtml.replace(
          /(?=<\/head>)|<script\s+src=['"][^'"]*uu5-environment\.js['"][^>]*>\s*<\/script>/i,
          m => {
            let result = `${m}
<script>
  ...
  var imports;
  if (typeof Uu5Loader !== "undefined") imports = Uu5Loader.config().imports;
  else if (typeof SystemJS !== "undefined") imports = SystemJS.getConfig().paths;
  if (imports) {
    var __devkitRemoteCdn = ${JSON.stringify(this.remoteCdnUrl)};
    for (var k in imports) {
      if (imports[k].slice(0, __devkitRemoteCdn.length) === __devkitRemoteCdn) {
        imports[k] = window.UU5.Environment.cdnBaseUri + imports[k].slice(__devkitRemoteCdn.length);
      }
    }
    if (typeof Uu5Loader !== "undefined") Uu5Loader.config({ imports: imports });
    else if (typeof SystemJS !== "undefined") SystemJS.config({ paths: imports });
  }
</script>`;
            return result;
          }
