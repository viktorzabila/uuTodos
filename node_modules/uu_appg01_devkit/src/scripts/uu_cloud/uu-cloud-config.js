...
class UuCloudConfig {
  ...
  uuAppBoxCopyConfig() {
    ...
    if ((match = result.appBoxCopyLocationUri.match(/^(.*)\/repository\/([^\/]+)\/?(.*)?\/?$/i))) {
      ...
      // directory
      if (match[3]) result.nexusDirectory = match[3] + "/";
    }
  ...
  async uuDeployConfig(includeAsid = true) {
    ...
    result.appBoxUri = this._getAppBoxUri();
    if (this.devkitConfig.appBoxCopyLocationUri) {
  ...
  _getAuthStrategy(taskName) {
    let strategy = this.devkitConfig[taskName + "AuthStrategy"];
  ...
  _getAppClientOpts() {
    let opts = {};
    if (this.devkitConfig.uuCloudBaseUri) {
      opts["uuC3ServerUri"] = this.devkitConfig.uuCloudBaseUri
    }
    if (this.devkitConfig.uuCloudLogStoreBaseUri) {
  ...
  _getAppBoxCopyLocationUri() {
    if (!this.devkitConfig.appBoxCopyLocationUri) return null;
